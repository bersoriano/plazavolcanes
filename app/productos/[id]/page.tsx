...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  ) : <div className="grid size-full place-items-center text-brand/30"><ImageIcon aria-hidden="true" className="size-16" /></div>}</div><div className="flex flex-col justify-center"><Link className="inline-flex w-fit items-center gap-2 rounded-full bg-accent px-3 py-1.5 text-sm font-semibold text-brand-hover" href={`/tiendas/${product.shop.slug}`}><Store aria-hidden="true" className="size-4" />{product.shop.name}</Link><h1 className="mt-5 font-display text-4xl font-semibold leading-tight tracking-[-0.04em] text-ink sm:text-5xl">{product.name}</h1><p className="mt-3 inline-flex w-fit rounded-full border border-line bg-surface px-3 py-1.5 text-sm font-semibold text-brand">{formatProductCondition(product.condition, product.used_condition)}</p><p className="mt-5 text-3xl font-semibold text-brand">{formatMxn(product.price_mxn)} <span className="text-sm font-medium text-muted">MXN</span></p><div className="my-7 h-px bg-line" /><p className="whitespace-pre-wrap text-base leading-8 text-muted">{product.description}</p><div className="mt-7"><ShareActions label="Compartir producto" title={product.name} /></div><p className="mt-8 rounded-2xl border border-line bg-surface p-4 text-sm leading-6 text-muted">Producto publicado por una tienda independiente de Plaza Volcanes.</p></div></div></section>;
}

...
export function CheckoutForm({ action, idempotencyKey }: { action: (state: ActionState, formData: FormData) => Promise<ActionState>; idempotencyKey: string }) {
  ...
  return <form action={formAction} className="space-y-4" noValidate><input name="idempotency_key" type="hidden" value={idempotencyKey} /><input name="country_code" type="hidden" value="MX" /><Field error={state.errors?.recipient?.[0]} label="Nombre de quien recibe" name="recipient" required /><Field error={state.errors?.address_line1?.[0]} label="Calle y número" name="address_line1" required /><Field error={state.errors?.address_line2?.[0]} label="Interior o referencia" name="address_line2" /><div className="grid gap-4 sm:grid-cols-2"><Field error={state.errors?.locality?.[0]} label="Ciudad o localidad" name="locality" required /><Field error={state.errors?.administrative_area?.[0]} label="Estado" name="administrative_area" required /></div><Field error={state.errors?.postal_code?.[0]} label="Código postal" name="postal_code" required /><Field error={state.errors?.delivery_instructions?.[0]} label="Instrucciones de entrega" name="delivery_instructions" /><Field error={state.errors?.buyer_note?.[0]} label="Nota para vendedor" name="buyer_note" />{state.message ? <p className="rounded-xl bg-sale/10 p-3 text-sm font-medium text-sale" role="status">{state.message}</p> : null}<CheckoutButton /></form>;
}
